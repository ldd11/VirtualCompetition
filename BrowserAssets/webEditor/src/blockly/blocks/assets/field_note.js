import { EVT_MUSIC } from '../../../utils/EventMsg';
/**
 * @license
...
const arrowButtonSvg = require('./arrow_button.svg');
...
Blockly.FieldNote = function (opt_value, opt_validator) {
  opt_value = opt_value && !isNaN(opt_value) ? String(opt_value) : '60';
...
Blockly.FieldNote.prototype.dispose_ = function () {
  ...
  return function () {
    ...
    thisField.mouseDownWrappers_.forEach((wrapper) => {
      Blockly.unbindEvent_(wrapper);
    });
    thisField.mouseEnterWrappers_.forEach((wrapper) => {
      Blockly.unbindEvent_(wrapper);
    });
...
Blockly.FieldNote.prototype.showEditor_ = function () {
  ...
  Blockly.DropDownDiv.setCategory('music_instrument');
...
Blockly.FieldNote.prototype.getPianoKeyPath_ = function (x, y, width, height) {
  return (
    ...
    + `Q${x} ${y + height} ${x + Blockly.FieldNote.KEY_RADIUS} ${y + height} `
    + `L${x + width - Blockly.FieldNote.KEY_RADIUS} ${y + height} `
    + `Q${x + width} ${y + height} ${x + width} ${y
      + height
      - Blockly.FieldNote.KEY_RADIUS} `
    + `L${x + width} ${y} `
    + `L${x} ${y}`
  );
};
...
Blockly.FieldNote.prototype.addOctaveButton_ = function (x, flipped, svg) {
  ...
  // console.log(`addOctaveButton ${Blockly.mainWorkspace.options.pathToMedia + Blockly.FieldNote.ARROW_SVG_PATH}`);
  Blockly.utils.createSvgElement(
    'line',
    {
      // 'stroke': this.sourceBlock_.getColourTertiary(),
...
Blockly.FieldNote.prototype.playNoteInternal_ = function () {
  if (Blockly.FieldNote.playNote_) {
    Blockly.FieldNote.playNote_(this.getValue(), 'music');
  }
};
...
Blockly.FieldNote.playNote_ = function (noteNum, id) {
  const event = new CustomEvent(EVT_MUSIC.MUSIC_PLAY_NOTE, {
    detail: {
      noteNum,
      category: id
    }
